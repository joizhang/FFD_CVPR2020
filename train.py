...
def main():
    ...
    train_data = DffdDataset(data_root=args.data_dir, mode='train', transform=transform, classes=classes, seed=args.seed)
    ...
    train_loader = DataLoader(train_data, num_workers=1, batch_size=args.batch_size, shuffle=True, drop_last=True,
                              pin_memory=True)
    val_data = DffdDataset(data_root=args.data_dir, mode='validation', transform=transform, classes=classes, seed=args.seed)
